...
__device__ void load_matrix_D_64(half * D,half * shmem,int blk_id,int NN,int H,int W,int C,int dc);
...
__device__ void load_matrix_D(half * D,half * shmem,int blk_id,int NN,int H,int W,int C,int dc,int version){
    ...
    if(version==128){
        ...
        load_matrix_D_64(D,shmem,blk_id,NN,H,W,C,dc);
    }
...
__device__ void load_matrix_D_64(half * D,half * shmem,int blk_id,int NN,int H,int W,int C,int dc){
    //calculate the initial position of data to be loaded
    int posi =(5120*blk_id+blockIdx.x*64)%(NN*H*W);
    int dN = posi/(H*W);
    posi=posi%(H*W)/64;
    int dh = posi/(W/8)*8;
    int dw = posi%(W/8)*8;
