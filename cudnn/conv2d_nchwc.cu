...
struct TrainingContext {
  ...
  size_t SetFwdConvolutionTensors(ConvolutionLayer& conv,
    ...
    checkCUDNN(cudnnSetTensor4dDescriptor(srcTensorDesc,
                                          ...
                                          CUDNN_DATA_INT8x4, n, c, h, w));

    checkCUDNN(cudnnSetFilter4dDescriptor(filterDesc, CUDNN_DATA_INT8x4,
                                          CUDNN_TENSOR_NCHW_VECT_C,
  ...
  void ForwardPropagation(void* data, void* result, void* weight,
                          void* workspace) {
    float alpha = 1.0f;
    float beta = 0.0f;
    checkCudaErrors(cudaSetDevice(m_gpuid));
    ...
    checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, dataTensor, data,
                                       conv1filterDesc, weight, conv1Desc,
...
void test(const Workload& wkl) {
  ...
  printf("%d, %d, %d, %d, %d, %d, %d, %f, %f GFLOPS\n",
         batch_size, width, channels, out_channels, kernel_size, pad, stride,
...
int main() {
  ...
  printf("N, H/W, CI, CO, KH/KW, padding, stride, time(s), GFLOPS\n");
