...
__device__ void move_data_int4(half *src,half *dst){
    //*((int4*)dst)=*((int4*)src);
...
__device__ void load_main_D(half *D,half *shmem,int H,int W){
    ...
    int offset_shmem=19+(threadIdx.x/16)*180+(lane_id/2)*18+(lane_id%2)*8;
    int offset_D=(threadIdx.x/16)*H*W+(lane_id/2)*W+(lane_id%2)*8;
...
__device__ void load_warp_0(half *D,half *shmem,int H,int W,int dh,int dw){
    ...
    //now fill boaders
    int colid=threadid%2;
    int rowid=threadid/2;

    //fill the op edge
    half *dst=shmem+1;
    half *src;
    if(dh==0){//fill zero, now at edge
        dst[colid*2+rowid*180]=static_cast<half>(0.);
        dst[1+colid*2+rowid*180]=static_cast<half>(0.);
    } else{
        src=D-W;
        dst[colid*2+rowid*180]=src[colid*2+rowid*H*W];
        dst[1+colid*2+rowid*180]=src[1+colid*2+rowid*H*W];
    }
    
    //fill the left edge
    dst=shmem+18;
    if(dw==0){//fill zero, now at edge
        dst[colid*36+rowid*180]=static_cast<half>(0.);
        dst[18+colid*36+rowid*180]=static_cast<half>(0.);
    } else {
        src=D-1;
        dst[colid*36+rowid*180]=src[colid*2*W+rowid*H*W];
        dst[18+colid*36+rowid*180]=src[W+colid*2*W+rowid*H*W];
    }
}
...
__device__ void load_warp_1_2(half *D,half *shmem,int H,int W,int dh,int dw,int warpid){
    ...
    if(warpid==1){
        ...
        if(dw==0||dh==0){//fill zero
           if(threadid<16){
                dst[threadid*180]=static_cast<half>(0.);
           }
        } else {
            src=D-1-W;
            if(threadid<16){
                dst[threadid*180]=src[threadid*H*W];
            }
        }
    } else {
        dst=shmem+17;
...
__device__ void im2col(half *shmem,int offset_D,int ker_id){
    ...
    half *src=shmem+ker_id/3*18+ker_id%3+threadIdx.x%2*1440+(threadIdx.x/2)/16*18+(threadIdx.x/2)%16;
