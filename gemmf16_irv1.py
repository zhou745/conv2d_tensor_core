...
def Gemm_ir_wmma(A,B,C):
    ...
    shmem = ib.allocate("float16", 24000, name="shmem",scope = "shared")
    #shmem_id = ib.allocate("int32", 256, name="shmem_id",scope = "shared")
    #sync thread model
    sync = tvm.call_extern("float32","__syncthreads")
    #define fragment 
    def_matrix_frag = tvm.call_extern("float32","__FRAGMENT_CH__")
    ...
    with ib.for_range(0,block_re,name ="b_for") as b_for:
        with ib.if_scope(bidx+num_block*b_for<block_num):
            ...
            warpid = tidx//32
            lane = tidx%32
            c_row_ele = 32*block_row_warp*warp_row_tile +shieft   #2 float 16 equals 1 float32
            row_ele_num = c_row_ele//2
            warp_offset_c_sh = warpid%block_row_warp*32*warp_row_tile+warpid/block_row_warp*16*c_row_ele
            #warp offset for C
            tile_offset_c = 16*rB
            ...
            with ib.for_range(0,4,name = "tile") as tile:
                ...
                read_c_int4a1 = tvm.call_extern("float32","__INT4READ__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16],Cp)
                ib.emit(read_c_int4a1)
                Cp = C.access_ptr("r",offset = block_offset_c+warp_offset_c+tile*16+lane//2*rB+lane%2*8+4)
                read_c_int4a2 = tvm.call_extern("float32","__INT4READ__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16+8],Cp)
                ib.emit(read_c_int4a2)
            ib.emit(sync)
            with ib.for_range(0,4,name = "tile") as tile:
                index0=0
                index1=tile
                load_matrix_frag_c1 = tvm.call_extern("float32","__LOADFRAG_C__",shmem[warp_offset_c_sh+tile*32],index0,index1,row_ele_num)
            ...
            with ib.for_range(0,4,name = "tile") as tile:
                ...
                read_c_int4b1 = tvm.call_extern("float32","__INT4READ__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16],Cp)
                ib.emit(read_c_int4b1)
                Cp = C.access_ptr("r",offset = block_offset_c+warp_offset_c+tile*16+lane//2*rB+lane%2*8+tile_offset_c+4)
                read_c_int4b2 = tvm.call_extern("float32","__INT4READ__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16+8],Cp)
                ib.emit(read_c_int4b2)
            ib.emit(sync)
            with ib.for_range(0,4,name = "tile") as tile:
                index0=1
                index1=tile
                load_matrix_frag_c2 = tvm.call_extern("float32","__LOADFRAG_C__",shmem[warp_offset_c_sh+tile*32],index0,index1,row_ele_num)
            ...
            with ib.for_range(0,cA//(chunk*16),name = "reduce_i") as reduce_i:
                ...
                with ib.for_range(0,chunk,name ="chunk_i",for_type = 'unroll') as chunk_i:
                    ...
                    with ib.for_range(0,warp_col_tile,name = 'col',for_type ='unroll') as col:
                        index0 = col
                        load_matrix_frag_a = tvm.call_extern("float32","__LOADFRAG_A_CH__",shmem[warp_offset_a_sh+16*chunk_i+col*16*sh_ele_num],index0,index2,row_ele_num)
                        ib.emit(load_matrix_frag_a)
                    with ib.for_range(0,warp_row_tile,name ='row',for_type = 'unroll') as row:
                        index1 = row
                        load_matrix_frag_b = tvm.call_extern("float32","__LOADFRAG_B_CH__",shmem[offset_b+warp_offset_b_sh+16*chunk_i+row*16*sh_ele_num],index1,index2,row_ele_num)
                ...
                with ib.for_range(0,chunk,name ="chunk_i",for_type = 'unroll') as chunk_i:
                    ...
                    with ib.for_range(0,warp_col_tile,name ="col",for_type='unroll') as col:
                        ...
                        with ib.for_range(0,warp_row_tile,name = "row",for_type='unroll') as row:
                            ...
                            #    ib.emit(load_matrix_frag_b)
                            wmma_compute = tvm.call_extern("float32","__WMMA_SYNC_CH__",index0,index1,index2)
            ...
            with ib.for_range(0,4,name = "tile") as tile:
                index0=0
                index1=tile
                store_matrix_frag_c1 = tvm.call_extern("float32","__STOREFRAG_C__",shmem[warp_offset_c_sh+tile*32],index0,index1,row_ele_num)
                ib.emit(store_matrix_frag_c1)
            #ib.emit(sync)
            with ib.for_range(0,4,name = "tile") as tile:
                #copy the data to shared memory by int4
                Cp = C.access_ptr("w",offset = block_offset_c+warp_offset_c+tile*16+lane//2*rB+lane%2*8)
                write_c_int4a1 = tvm.call_extern("float32","__INT4WRITE__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16],Cp)
                ib.emit(write_c_int4a1)
                Cp = C.access_ptr("w",offset = block_offset_c+warp_offset_c+tile*16+lane//2*rB+lane%2*8+4)
                write_c_int4a2 = tvm.call_extern("float32","__INT4WRITE__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16+8],Cp)
                ib.emit(write_c_int4a2)
            #ib.emit(sync)
            with ib.for_range(0,4,name = "tile") as tile:
                index0=1
                index1=tile
                store_matrix_frag_c2 = tvm.call_extern("float32","__STOREFRAG_C__",shmem[warp_offset_c_sh+tile*32],index0,index1,row_ele_num)
            ...
            with ib.for_range(0,4,name = "tile") as tile:
                ...
                write_c_int4b1 = tvm.call_extern("float32","__INT4WRITE__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16],Cp)
                ib.emit(write_c_int4b1)
                Cp = C.access_ptr("w",offset = block_offset_c+warp_offset_c+tile*16+lane//2*rB+lane%2*8+tile_offset_c+4)
                write_c_int4b2 = tvm.call_extern("float32","__INT4WRITE__",shmem[warp_offset_c_sh+tile*32+lane//2*c_row_ele+lane%2*16+8],Cp)
                ib.emit(write_c_int4b2)
            ib.emit(sync)
...
if __name__ == "__main__":
    ...
    with tvm.target.create('cuda'):
        ...
        C = tvm.extern((rA,rB),[A,B],lambda ins,outs:Gemm_ir_wmma(ins[0],ins[1],outs[0]),name = "ir_wmma",dtype = 'float32')
        assert(cA==cB)
