...
def convolutionf16(D,F,temp,O):
    ...
    with ib.for_range(0,loop_len,name ="blk_id") as blk_id:
        with ib.if_scope(bidx+blk_id*block_num<(rD*rF)/(block_zise*block_zise)):
            ...
            load_first_data = tvm.call_extern("float32","load_matrix_D",Dp,shmem,blk_id,H,W,C,0)
            ...
            load_filter=tvm.call_extern("float32","load_matrix_F",Fp,shmem,offset_F_warp,blk_id,K,C,H,W,tidx%2*8,0)
            ib.emit(load_filter)
            ...
            im2col=tvm.call_extern("float32","im2col",shmem,offset_D_warp,0)
            ib.emit(im2col)
            ib.emit(sync)
            ...
            with ib.for_range(0,C/16,name="c_id",for_type = 'unroll') as c_id:
                with ib.for_range(0,9,name="ker_id",for_type = 'unroll') as ker_id:
                    ...
                    with ib.if_scope(ker_id<8):
                        ...
                        load_filter=tvm.call_extern("float32","load_matrix_F",Fp,shmem,offset_F_warp,blk_id,K,C,H,W,c_id*16+tidx%2*8,ker_id+1)
                        ib.emit(load_filter)
                        #load data for next iteration
                        im2col=tvm.call_extern("float32","im2col",shmem,offset_D_warp,ker_id+1)
                        ib.emit(im2col)
                    ib.emit(sync)
     
                with ib.if_scope(c_id<C/16-1):
                    #load the next 9 iteration data from global memory
                    load_data = tvm.call_extern("float32","load_matrix_D",Dp,shmem,blk_id,H,W,C,c_id*16+16)
                    ib.emit(load_data)

                    #load filter for next cd iter
                    load_filter=tvm.call_extern("float32","load_matrix_F",Fp,shmem,offset_F_warp,blk_id,K,C,H,W,c_id*16+16+tidx%2*8,0)
                    ib.emit(load_filter)
                    ib.emit(sync)
            ...
            ib.emit(sync)
            Op=O.access_ptr("w")
            store_O=tvm.call_extern("float32","store_output",Op,shmem,blk_id,P,Q,K)
            ib.emit(store_O)
            ib.emit(sync)
    body = ib.get()
    return(body)
...
if __name__ == "__main__":
    ...
    with tvm.target.create('cuda'):
        ...
        for index in range(16):
            dataf.append(a_np[0][index][255][242])
        #print(dataf)
        ...
        for id in range(256):
            temp=(1.0*result[0][0][254][id]-1.0*result2[0][0][254][id])/result2[0][0][254][id]
            if temp>0.1:
                print(id,result[0][0][254][id],result2[0][0][254][id])
