...
def convolutionf16(D,F,LOAD_INDEX_D,LOAD_INDEX_F,O):
    ...
    shmem_D = ib.allocate("float16", 2048, name="shmem_D",scope = "shared")

    shmem_F = ib.allocate("float16", 2048, name="shmem_F",scope = "shared")
    ...
    with ib.for_range(0,loop_len,name ="blk_id") as blk_id:
        with ib.if_scope(bidx+blk_id*block_num<rD*rF//block_len//block_len):
            ...
            Dp = D.access_ptr("r")
            Fp = F.access_ptr("r")
            LDp = LOAD_INDEX_D.access_ptr("r",offset =1)
            LFp = LOAD_INDEX_F.access_ptr("r",offset =1)
            offset_sh = 0
            load_D_matrix = tvm.call_extern("float32","load_matrix",Dp,shmem_D,LDp,offset_sh,0)
            ib.emit(load_D_matrix)
            offset_sh = 0
            load_F_matrix = tvm.call_extern("float32","load_matrix",Fp,shmem_F,LFp,offset_sh,0)           
...
def compute_index(ind):
    for cols in range(N*P*Q):
        ...
        for rows in range(C*R*S):
            ...
            dH+=dR
            dW+=dS
            if(dH>=0 and dW>=0 and dH<H and dW<W):
                ind[cols][rows] = dN*C*H*W+dC*H*W+dH*W+dW
...
if __name__ == "__main__":
    ...
    with tvm.target.create('cuda'):
        ...
        LOAD_INDEX_D = tvm.placeholder((rD,cD),dtype = 'int32')
        LOAD_INDEX_F = tvm.placeholder((rF,cF),dtype = 'int32')
        ...
        d1_np = -1*np.ones((rD,cD),dtype = np.int32)
        d2_np = -1*np.ones((rF,cF),dtype = np.int32)
